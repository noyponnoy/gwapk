...
//        }
	...
	public void scheduleJob(String id, long ms)
	{
		synchronized (this)
		{
			...
			{	/* update the alarm if the job has to be executed before all others */
				...
				scheduleExactAlarmSafe(job.Time, pending);
			}
		}
	}
	...
	@RequiresApi(api = Build.VERSION_CODES.M)
	private void scheduleExactAlarmSafe(long triggerAtMillis, PendingIntent pending)
	{
		try
		{
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S)
			{
				if (mManager.canScheduleExactAlarms())
				{
					mManager.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, triggerAtMillis, pending);
				}
				else
				{
					/* Fall back to inexact alarm if exact alarm permission not granted */
					mManager.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, triggerAtMillis, pending);
				}
			}
			else
			{
