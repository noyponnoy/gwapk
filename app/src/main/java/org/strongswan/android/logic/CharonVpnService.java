...
//                }
    ...
    public class BuilderCache {
        ...
        public BuilderCache(VpnProfile profile) {
            ...
            /* GW VPN Split Tunneling: profiles generated from the server list do not
             * carry their own app selection, so apply the mode the user configured in
             * Settings → Split Tunneling (Off / Only selected / All except selected).
             * The state is read at connect time; changes made while the tunnel is up
             * apply on reconnect. SplitTunnelStore.getEffectiveMode() already collapses
             * degenerate states (empty selection lists) to OFF, so an empty allow list
             * can never accidentally capture the whole device. */
            if (appHandling == SelectedAppsHandling.SELECTED_APPS_DISABLE) {
                com.witvpn.ikev2.features.splittunnel.SplitTunnelMode stMode =
                        com.witvpn.ikev2.features.splittunnel.SplitTunnelStore.getEffectiveMode(CharonVpnService.this);
                if (stMode == com.witvpn.ikev2.features.splittunnel.SplitTunnelMode.EXCEPT_SELECTED) {
                    appHandling = SelectedAppsHandling.SELECTED_APPS_EXCLUDE;
                    mSelectedApps.clear();
                    mSelectedApps.addAll(
                            com.witvpn.ikev2.features.splittunnel.SplitTunnelStore.getDisallowedPackagesSorted(CharonVpnService.this));
                } else if (stMode == com.witvpn.ikev2.features.splittunnel.SplitTunnelMode.ONLY_SELECTED) {
                    appHandling = SelectedAppsHandling.SELECTED_APPS_ONLY;
                    mSelectedApps.clear();
                    mSelectedApps.addAll(
                            com.witvpn.ikev2.features.splittunnel.SplitTunnelStore.getAllowedPackagesSorted(CharonVpnService.this));
                }
            }
