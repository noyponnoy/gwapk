...
public class VpnProfileControlActivity extends AppCompatActivity
{
	...
	public static final String SILENT_OPTION = "org.strongswan.android.action.SILENT_OPTION";
	...
	private void disconnect(Intent intent)
	{
		...
		if (mService != null)
		{
				...
				mService.getState() == State.CONNECTING)
			{
				...
				boolean isSilent = intent.getBooleanExtra(SILENT_OPTION, false);
				if (isSilent) {
					mService.disconnect();
					finish();
				} else {
					...
					try {
						...
						/* Activity state already saved, just disconnect silently */
						mService.disconnect();
						finish();
					}
				}
